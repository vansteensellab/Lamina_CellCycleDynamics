...
Simple base-R plotting. Data to plot (HCT116) :
...
# 
# Prepare output
output_dir <- "ts191212_figure_track_HCT116_replication"
dir.create(output_dir, showWarnings = FALSE)

# Load the data sets required
file_names <- c("g1", "s", "g2", "repliseq")

repliseq <- import("ts190731_4DN_RepliSeq/bigwigs/HCT116_combined_20kb.bw")
g1 <- import("results/tracks/normalized/bin-20kb/pADamID-HCT116_G1_LMNB2-20kb-combined.bw")
s <- import("results/tracks/normalized/bin-20kb/pADamID-HCT116_S_LMNB2-20kb-combined.bw")
g2 <- import("results/tracks/normalized/bin-20kb/pADamID-HCT116_G2_LMNB2-20kb-combined.bw")
...
PlotDataTracks(bins, 
               chr = "chr1", start = 52e6, end = 70e6)

PlotDataTracks(bins, 
               chr = "chr5", start = 129e6, end = 156e6)

PlotDataTracks(bins, 
               chr = "chr9", start = 86e6, end = 102e6)

PlotDataTracks(bins, 
               chr = "chr10", start = 73e6, end = 81e6)

PlotDataTracks(bins, 
               chr = "chr11", start = 0e6, end = 25e6)
...
PlotDataTracks(bins_difference, 
               chr = "chr1", start = 52e6, end = 70e6, ylim = c(-2, 2))

# Add smoothing, similar to analysis
bins_difference <- SmoothData(bins_difference, size = 10e5)

PlotDataTracks(bins_difference, 
               chr = "chr1", start = 52e6, end = 70e6, ylim = c(-0.5, 0.5))




PlotDataTracks(bins, 
               chr = "chr1", start = 212e6, end = 223e6)

PlotDataTracks(bins_difference, 
               chr = "chr1", start = 212e6, end = 223e6, ylim = c(-0.5, 0.5))
