---
title: "Cell cycle sorting - differential LADs - HCT116"
...
Differential LADs between G1 and G2 sorted cells, with 20kb normalized (& 
scaled) bins.
...
# 
# Prepare output
output_dir <- paste0("ts190708_differential_analysis_BinsandLADs_", cell)
...
samples.df$phase <- sapply(samples.df$name, function(x) strsplit(x, "_")[[1]][2])
samples.df$phase <- factor(samples.df$phase, levels = c("G1", "S", "G2"))
...
LADs_G1 <- GetHMMRegion(df.hmm, "G1", centromeres = centromeres)
LADs_S <- GetHMMRegion(df.hmm, "S", centromeres = centromeres)
LADs_G2 <- GetHMMRegion(df.hmm, "G2", centromeres = centromeres)

# Combine cell-phase LADs into one consensus set
LADs <- union(LADs_G1, LADs_G2)
...
```{r plot distributions, fig.width = 4.5, fig.height = 4}

# Plot the distributions for the various samples
plot(1, 1, type = "n", xlim = c(-5, 5), ylim = c(0, 0.4),
...
```{r scaling of the data, fig.width = 4.5, fig.height = 4}
...
plot(1, 1, type = "n", xlim = c(-3, 3), ylim = c(0, 0.55),
     xlab = "Dam-ratio (log2)", ylab = "density", main = "Density Dam ratio for bins")
...
```{r variance modeling, fig.width = 4.5, fig.height = 4}

# Model variance for each bin, based on various parameters
design <- model.matrix(~ 0 + samples.df$phase + samples.df$replicate)
...
# Get the results
results <- decideTests(fit, adjust.method = "BH", p.value = 0.05, method = "global")
...
```{r visualize results LADs, fig.width = 5, fig.height = 3.5}
...
```{r cLADs, fig.width = 5, fig.height = 3.5}
...
# Absolute scores
ggplot(df.melt, aes(x = variable, y = value / 50, fill = class)) + 
...
df.new <- data.frame(result = df$result,
                     Dam = log2((rowMeans(df[, grep("G2_Dam", names(df))],
                                    na.rm = T) + 1) / 
                       (rowMeans(df[, grep("G1_Dam", names(df))],
                                na.rm = T) + 1)),
                     LMNB2 = log2((rowMeans(df[, grep("G2_LMNB2", names(df))],
                                    na.rm = T) + 1) / 
                       (rowMeans(df[, grep("G1_LMNB2", names(df))],
                                na.rm = T) + 1)))
                     
df.new <- melt(df.new, id.vars = "result")

ggplot(df.new, aes(x = result, y = value, fill = variable)) +
  # geom_quasirandom() +
  geom_boxplot(outlier.shape = NA, col = "black", width = 0.7, position = "dodge") +
  ggtitle("Target ratios") +
  xlab("target") +
  ylab("ratio G2/G1 (log2)") +
