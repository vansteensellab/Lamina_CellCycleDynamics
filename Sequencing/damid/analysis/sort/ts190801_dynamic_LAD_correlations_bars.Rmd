---
title: "Cell cycle sorting - correlations of dynamic LADs"
...
```{r rnaseq, fig.width = 4, fig.height = 3}
...
```{r repliseq, fig.width = 4, fig.height = 3}
...
# Also, I want to test if early-replicating DNA is always higher in G1 and 
...
```{r LAD characteristics, fig.width = 4, fig.height = 3}
...
```{r telomeres and centromeres, fig.width = 4, fig.height = 3}
...
```{r extra figures telomere distance, fig.width = 9, fig.height = 7}
...
plt <- ggplot(df.combined, aes(x = distance_to_telomeres, y = G2)) +
    geom_point(aes(col = seqnames), #alpha = 0.5, 
               size = 1) +
    geom_smooth(method = "loess", se = F) +
    geom_hline(yintercept = 0, col = "grey", linetype = "dashed") +
...
```{r telomeres per time point, , fig.width = 9, fig.height = 7}
...
```{r cLADs, fig.width = 4, fig.height = 3}
